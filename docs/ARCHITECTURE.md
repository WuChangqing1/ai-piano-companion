...
| 后端 | Python 3.10+ / FastAPI | REST API 服务 |
| 数据库 | SQLite 3 + SQLAlchemy 2.x | 本地持久化（用户、曲谱、报告） |
| AI - 手型 | MediaPipe（Mock 兜底） | 手部关键点检测 |
| AI - 音频 | ByteDance Piano Transcription（Mock 兜底） | 音频转 MIDI |
| AI - 曲谱 | Oemer OMR（Mock 兜底） | 光学乐谱识别 |
...
│   │   ├── audio_amt.py        # 音频转录（AMT / Mock）
│   │   ├── omr_parser.py       # 曲谱识别（OMR / Mock）
